<x-app-layout>
    <x-slot name="header">
        <div>
            ...
            <h2 class="mt-1 font-semibold text-xl text-gray-900 dark:text-white leading-tight">Nuevo producto</h2>
        </div>
    </x-slot>
                ...
                <form method="POST" action="{{ route('admin.products.store') }}" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <select id="unit_type" name="unit_type" class="mt-1 block w-full rounded-lg border border-gray-200/80 bg-white/70 px-3 py-2 shadow-sm backdrop-blur focus:border-indigo-500 focus:ring-indigo-500 dark:border-gray-800/80 dark:bg-gray-950/30">
                                <option value="unit" {{ old('unit_type', 'unit') === 'unit' ? 'selected' : '' }}>Pieza</option>
                                <option value="weight" {{ old('unit_type') === 'weight' ? 'selected' : '' }}>Peso</option>
                            </select>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <x-text-input id="stock" name="stock" type="number" step="0.001" min="0" class="mt-1 block w-full" value="{{ old('stock') }}" />
                        ...
                        <div class="flex items-end">
                            <label class="inline-flex items-center gap-2">
                                ...
                                <input type="checkbox" name="is_active" value="1" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500 dark:border-gray-700 dark:bg-gray-950/30" {{ old('is_active', '1') ? 'checked' : '' }}>
